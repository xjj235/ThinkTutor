...
it("describes missing concepts and coach questions without confusing them with student quotations", async () => {
  const draft = await validDraft();
  draft.gaps = [{ title: "不利方向待补充", evidence: "教练追问了净头寸，学生未说明不利影响的条件。", repairTask: "请说明方向。", priority: 5 }];
  expect(() => assertReportGrounding(draft, [answer])).not.toThrow();
...
it.each([
  ...
  "“完全虚构的回答”表明仍有漏洞",
  "学生说：“引文未结束",
])("rejects fabricated gap quotations regardless of attribution phrasing: %s", async (evidence) => {
  const draft = await validDraft();
  draft.gaps[0].evidence = evidence;
  expect(() => assertReportGrounding(draft, [answer])).toThrow("学生原文");
});

it("accepts a genuine student gap quotation without requiring a magic attribution prefix", async () => {
  const draft = await validDraft();
  draft.gaps[0].evidence = `学生回答：“${answer}”，尚未说明变化的不利方向。`;
  expect(() => assertReportGrounding(draft, [answer])).not.toThrow();
});
