...
describe("learning session integration flow", () => {
  ...
  it("creates a session, completes the loop, and creates a retry session", async () => {
    const created = await createLearningSession(task);
    ...
    expect(afterDiagnosis.session.socraticRound).toBe(0);
    ...
    expect(round1.session.socraticRound).toBe(1);
    ...
    expect(round2.session.socraticRound).toBe(2);

    const round3 = await submitLearningAnswer(created.session.id, {
      answer: "前提是机构之间存在共同风险敞口，因此局部冲击会放大。",
      clientRequestId: "answer-round-3",
    });
    expect(round3.session.phase).toBe("FEYNMAN");
    expect(round3.session.socraticRound).toBe(3);
    ...
    expect(completed.report.scores.overallScore).toBeGreaterThan(0);
