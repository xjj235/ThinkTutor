...
async function resolveTaskForUser(userId: string, input: CreateSessionInput): Promise<{ task: CreateSessionInput; source: "SELF_DIRECTED" | "ASSIGNMENT"; assignmentId?: string; existingSessionId?: string }> {
  ...
  const usedAttempts = await prisma.learningSession.count({
...
export async function submitLearningAnswer(sessionId: string, input: { answer: string; clientRequestId: string }): Promise<SessionPayload & { duplicate: boolean }> {
  ...
  if (session.knowledgeRuntime && knowledgeRuntimeSchema.parse(session.knowledgeRuntime).v12) return submitV12Turn(sessionId, input.answer, input.clientRequestId);
...
export async function requestHint(sessionId: string, input: { clientRequestId: string }): Promise<SessionPayload & { duplicate: boolean }> {
  ...
  if (session.knowledgeRuntime && knowledgeRuntimeSchema.parse(session.knowledgeRuntime).v12) return submitV12Turn(sessionId, "", input.clientRequestId, false, true);
...
export async function submitFeynmanExplanation(sessionId: string, input: { explanation: string; clientRequestId: string }): Promise<{ payload: SessionPayload; report: LearningReportDTO | null; duplicate: boolean }> {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (changed.count !== 1) throw new AppError("CONFLICT", "会话状态已变化，请刷新后重试。", 409, true);
      await tx.message.create({ data: { id: explanationMessageId, sessionId, role: "USER", phase: "FEYNMAN", content: input.explanation, clientRequestId: input.clientRequestId } });
