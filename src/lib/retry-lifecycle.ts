...
import { finalizeRetryReview, RETRY_REVIEW_MESSAGE_LIMIT, retryReviewInputSchema, type RetryReview, type RetryReviewInput } from "./retry-review";
...
export async function reviewCompletedRetry(
  ...
  // v1.2 may need more verification turns than the model review window. Keep
  // exact recent evidence and the independent explanation within that window,
  // but never claim mastery when earlier potentially conflicting evidence is omitted.
  const partialHistory = input.messages.length > RETRY_REVIEW_MESSAGE_LIMIT;
  const messages = input.messages.slice(-RETRY_REVIEW_MESSAGE_LIMIT);
  const independentExplanation = input.messages.filter((message) => message.isIndependentExplanation).at(-1);
  if (independentExplanation && !messages.some((message) => message.id === independentExplanation.id)) {
    messages.shift();
    messages.unshift(independentExplanation);
  }
  let resolutionBlockedReason = partialHistory ? "本次复核仅覆盖部分学习历史，暂不能完整核对早期表达与后续证据，请继续巩固。" : undefined;
