...
export function AdminUserActions({ id, name, role, status, isCurrentUser }: { id: string; name: string; role: Role; status: Status; isCurrentUser: boolean }) {
  ...
  async function patch(kind: "role" | "status", value: Role | Status, confirmed = false): Promise<void> {
    ...
    if (dangerous && !confirmed && !window.confirm(`确认将“${name}”的${kind === "role" ? "角色" : "状态"}改为 ${value} 吗？${value === "DISABLED" || value === "DELETED" ? "该用户现有登录会话将被撤销。" : ""}`)) return;
  ...
  return <div className="action-stack"><div className="inline-controls"><label><span className="sr-only">{name}的角色</span><select key={`role-${selectionRevision}`} aria-label={`${name}的角色`} disabled={pending || currentStatus === "DELETED"} value={currentRole} onChange={(event) => {const value=event.target.value as Role;if(value==="ADMIN"&&!window.confirm(`确认将“${name}”的角色改为 ADMIN 吗？`)){setSelectionRevision((revision)=>revision+1);return}void patch("role",value,true)}}><option>STUDENT</option><option>TEACHER</option><option>ADMIN</option></select></label><label><span className="sr-only">{name}的状态</span><select key={`status-${selectionRevision}`} aria-label={`${name}的状态`} disabled={pending || isCurrentUser} value={currentStatus} onChange={(event) => {const value=event.target.value as Status;if(value!=="ACTIVE"&&!window.confirm(`确认将“${name}”的状态改为 ${value} 吗？该用户现有登录会话将被撤销。`)){setSelectionRevision((revision)=>revision+1);return}void patch("status",value,true)}}><option>ACTIVE</option><option>DISABLED</option><option>DELETED</option></select></label></div>{isCurrentUser ? <small>不能停用当前管理员账号。</small> : null}<AsyncFeedback state={feedback} onRetry={retry} /></div>;
}
