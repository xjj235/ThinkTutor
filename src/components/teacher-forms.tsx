...
export function CourseCreateForm(){const router=useRouter();const[pending,setPending]=useState(false);const[message,setMessage]=useState("");return <form className="stack-form card catalog-form" onSubmit={async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();setPending(true);setMessage("");const f=new FormData(e.currentTarget);try{const data=await postJson("/api/courses",{title:String(f.get("title")),description:String(f.get("description"))||undefined,subject:String(f.get("subject"))||undefined,audience:String(f.get("audience"))||undefined});router.push(`/teacher/courses/${data?.id}`);router.refresh()}catch(error){setMessage(`错误：${error instanceof Error?error.message:"创建失败"}`)}finally{setPending(false)}}}><label>课程名称<input name="title" required minLength={2} maxLength={curriculumTextLimits.courseTitle}/></label><label>学科<input name="subject" maxLength={100}/></label><label>适合人群<input name="audience" maxLength={200}/></label><label>课程说明<textarea name="description" rows={5} maxLength={curriculumTextLimits.description}/></label><Status message={message}/><button className="button" disabled={pending}>{pending?"创建中…":"创建课程"}</button></form>}

export function ChapterCreateForm({courseId}:{courseId:string}){const router=useRouter();const[pending,setPending]=useState(false);const[message,setMessage]=useState("");return <form className="stack-form card catalog-form" onSubmit={async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();setPending(true);setMessage("");const formElement=e.currentTarget;const f=new FormData(formElement);try{await postJson(`/api/courses/${courseId}/chapters`,{title:String(f.get("title")),description:String(f.get("description"))||undefined,sortOrder:Number(f.get("sortOrder"))});setMessage("章节已创建。");formElement.reset();router.refresh()}catch(error){setMessage(`错误：${error instanceof Error?error.message:"创建失败"}`)}finally{setPending(false)}}}><h2>添加章节</h2><label>章节名称<input name="title" required minLength={2} maxLength={curriculumTextLimits.chapterTitle}/></label><label>顺序<input name="sortOrder" type="number" defaultValue="1" min="0" max="10000" required/></label><label>说明<textarea name="description" rows={3} maxLength={curriculumTextLimits.description}/></label><Status message={message}/><button className="button" disabled={pending}>添加章节</button></form>}

export function GoalCreateForm({chapterId}:{chapterId:string}){const router=useRouter();const[pending,setPending]=useState(false);const[message,setMessage]=useState("");return <form className="stack-form card catalog-form" onSubmit={async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();setPending(true);setMessage("");const formElement=e.currentTarget;const f=new FormData(formElement);try{await postJson(`/api/chapters/${chapterId}/goals`,{title:String(f.get("title")),objective:String(f.get("objective")),sortOrder:Number(f.get("sortOrder")),expectedLevel:String(f.get("expectedLevel"))||undefined});setMessage("目标已创建。");formElement.reset();router.refresh()}catch(error){setMessage(`错误：${error instanceof Error?error.message:"创建失败"}`)}finally{setPending(false)}}}><h3>添加学习目标</h3><label>目标标题<input name="title" required minLength={2} maxLength={curriculumTextLimits.goalTitle}/></label><label>可验证目标<textarea name="objective" required minLength={10} maxLength={curriculumTextLimits.goalObjective} rows={3}/></label><label>预期水平<input name="expectedLevel" maxLength={curriculumTextLimits.learnerLevel}/></label><label>顺序<input name="sortOrder" type="number" defaultValue="1" min="0" max="10000" required/></label><Status message={message}/><button className="button button-secondary" disabled={pending}>添加目标</button></form>}
...
export function AssignmentCreateForm({ classrooms }: { classrooms: AssignmentClassroom[] }) {
  ...
  return (
    ...
    >
      ...
      <label>任务名称<input name="title" required minLength={2} maxLength={curriculumTextLimits.assignmentTitle} /></label>
      <label>学习要求<textarea name="instructions" required minLength={10} maxLength={curriculumTextLimits.assignmentInstructions} rows={5} /></label>
      <label>补充说明<textarea name="description" rows={3} maxLength={curriculumTextLimits.description} /></label>
      <label>学习者水平<input name="learnerLevel" defaultValue="入门" required maxLength={curriculumTextLimits.learnerLevel} /></label>
